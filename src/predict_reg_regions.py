...
regions_type = ['active_enchancer', 'rep_enhancer', 'promoter']
bed_files = ['vars_in_acitive_enchancers.bed', 'vars_in_repressed_enchancers.bed', 'variants_in_active_promoters.bed']

for h,r in enumerate(regions_type):
    ...
    cmd_bed = "bedtools intersect -a ../results/peaks_set.bed -b ../data/"+ bed_files[h] + " -wa -wb > ../results/variants/" + r + "_overlap.bed"
    subprocess.run(cmd_bed, shell=True)

    active_enhancer = pd.read_csv("../results/variants/" + r + "_overlap.bed", header=None, sep='\t', names=["chr_peak", "start_peak", "end_peak", "chr_var", "pos_var", "end_var"])
    ...
    with open('../results/variants/' + r + '.pkl', 'wb') as file:
    ...
    with open('../results/variants/target_'+ r + '.pkl', 'wb') as file:
    ...
    with open('../results/variants/pred_profile_'+ r + '.pkl', 'wb') as file:
        pickle.dump(profile_ref, file)

    with open('../results/variants/pred_count_'+ r + '.pkl', 'wb') as file:
        pickle.dump(count_ref, file)

    with open('../results/variants/alt_pred_profile_'+ r + '.pkl', 'wb') as file:
        pickle.dump(profile_alt, file)

    with open('../results/variants/alt_pred_count_'+ r + '.pkl', 'wb') as file:
    ...
    with open("../results/variants/pred_profile_"+ r + ".pkl", 'rb') as file:
        profile = pickle.load(file)
    with open("../results/variants/pred_count_"+ r + ".pkl", 'rb') as file:
        count = pickle.load(file)

    with open("../results/variants/alt_pred_profile_"+ r + ".pkl", 'rb') as file:
        alt_profile = pickle.load(file)
    with open("../results/variants/alt_pred_count_"+ r + ".pkl", 'rb') as file:
    ...
    for i in range (count.size(0)):
        for t in range(4):
            ...
            if diff > 0.17661169 or diff < -0.20696962:
                which_seq.append(i)
    ...
    with open('../results/variants/' + r + '_diff_var.pkl', 'wb') as file:
        pickle.dump(active_enhancer, file)
